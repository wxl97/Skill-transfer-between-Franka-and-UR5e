...
bool CartesianPoseExampleController::init(hardware_interface::RobotHW* robot_hardware,
  ...
  target_pose_subscriber_ = node_handle.subscribe<geometry_msgs::PoseStamped>(
    "/target_pose", 1, &CartesianPoseExampleController::targetPoseCallback, this);
...
void CartesianPoseExampleController::update(const ros::Time& /* time */,
  ...
  if (received_target_pose_) {
    new_pose = target_pose_;
  } else {
    ROS_ERROR_STREAM("No target pose!!!");
    new_pose = initial_pose_;  // 如果还没收到指令，就保持原位
  }
...
void CartesianPoseExampleController::targetPoseCallback(const geometry_msgs::PoseStamped::ConstPtr& pose_msg) {
  ...
  // 将矩阵按列优先（column-major）存储
  Eigen::Matrix4d matrix = transform.matrix();
  int index = 0;
  for (int col = 0; col < 4; ++col) {
    for (int row = 0; row < 4; ++row) {
      target_pose_[index++] = matrix(row, col);
    }
  }

  received_target_pose_ = true;
}
